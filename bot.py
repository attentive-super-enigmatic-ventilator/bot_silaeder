        ...
        if event.type == vk_api.longpoll.VkEventType.MESSAGE_NEW and event.to_me and str(event.user_id) in admins:
            ...
            if users[event.user_id] == 77:
                ...
                for i in range(len(numb)):
                    ...
                    row_cells[1].text = downloading_google_sheet.sheet1[numb[i] + 1][
                        1]  # взятая из таблицы информация о каждом учителе
                    row_cells[2].text = downloading_google_sheet.sheet1[numb[i] + 1][
                        0]  # взятая из таблицы информация о каждом учителе
                    row_cells[3].text = downloading_google_sheet.sheet1[numb[i] + 1][
                        2]  # взятая из таблицы информация о каждом учителе
                    row_cells[4].text = downloading_google_sheet.sheet1[numb[i] + 1][
                        3]  # взятая из таблицы информация о каждом учителе
            ...
            if 'привет' in text or 'начать' in text:
                if vko.users.get(user_ids=event.user_id, fields=['sex'])[0]['sex'] == 2:
                    vko.messages.send(user_id=event.user_id,
                                      ...
                                      message='Привет, ' + (inf['first_name']) + ' ' + inf[
                                          'last_name'] + '!' + ' Я рад, что ты мне написал!',
                                      keyboard=create_keyb1(['Узнать расписание']))
                else:
                    vko.messages.send(user_id=event.user_id,
                                      random_id=random.randint(1, 109),
                                      message='Привет, ' + (inf['first_name']) + ' ' + inf[
                                          'last_name'] + '!' + ' Я рад, что ты мне написала!',
                                      keyboard=create_keyb1(['Узнать расписание']))
                    incorrect_command = False
            if text == 'отмена':
                vko.messages.send(user_id=event.user_id,
